...
const EducatedCityIcon = ({ size, unit, onMouseEnter, onMouseLeave, hoverable }) => {

  const wh = `${size>0?size*0.8:1}${unit?unit:'em'}`; // Width and Height

  const childStyle = { gridColumn: 1, gridRow: 1 }

  return (
    <AchievementFrame hoverable={hoverable} size={size} unit={unit} backgroundColor='darkcyan' onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} >
      <svg width={wh} height={wh} viewBox="0 0 448 512" style={{...childStyle, transform: 'scale(0.7) translateY(3%) translateX(18%)'}} >
